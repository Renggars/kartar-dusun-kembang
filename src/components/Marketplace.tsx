...
// --- KOMPONEN SKELETON CARD (TIDAK BERUBAH) ---
const MarketplaceCardSkeleton = () => (
  // Gunakan aspect-4/5 yang sama dengan kartu asli
...
export default function Marketplace({ limit }: { limit?: number }) {
  ...
  const [selectedCategory, setSelectedCategory] =
    useState<CategoryFilter>("Semua"); // --- Fetch Data ---

  const {
    data: items = [],
    isLoading,
    isFetching,
  } = trpc.marketplace.list.useQuery(); // Ambil isFetching
  ...
  const finalData = limit ? filtered.slice(0, limit) : filtered; // Tentukan apakah kita harus menampilkan skeleton. Kita tampilkan jika: // 1. isLoading (fetch pertama) // 2. isFetching (refetch setelah filter atau data lama) DAN finalData kosong

  const shouldShowSkeleton =
    isLoading || (finalData.length === 0 && isFetching);
  const skeletonCount = limit || 8; // Jumlah skeleton default
  ...
  return (
    ...
    >
      <div className="container mx-auto sm:px-6 lg:px-12">
        ...
        {/* --- PRODUCT GRID / SKELETON (2 Col Mobile, 4 Col Desktop) --- */}
        <motion.div
          layout
          className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-8 md:gap-8 px-4"
        >
          {/* TAMPILKAN SKELETON JIKA SEDANG LOADING/FETCHING */}
          {shouldShowSkeleton ? (
            <>
            ...
            /* TAMPILKAN DATA ASLI SETELAH SELESAI */
        ...
        {/* LOADING SPINNER (Tampil bersama Skeleton di Grid) */}
        {shouldShowSkeleton && (
