...
// Definisikan tipe untuk StatCardProps (Tidak Berubah)
...
export default function DashboardPage() {
  ...
  const latestMessage =
    messages && messages.length > 0
      ? messages.sort(
        ...
        )[0] // Asumsi ada field createdAt dan ini adalah struktur pesan
      : null;
  ...
  // 4. Pengguna (Asumsi query user tersedia dan mengembalikan array user)
  // const { data: users, isLoading: isLoadingUsers } =
  //   trpc.user.getAll.useQuery(); // Menggunakan query user statis yang diminta
  // const memberCount = users?.length || 0;

  // --- DATA STATISTIK TERINTEGRASI ---
  const statsData = [
    {
      icon: Mail,
  ...
  // Asumsi program memiliki field title, date, dan mungkin image/imageUrl.
  const latestPrograms =
    programs?.slice(0, 4).map((program, index) => ({
      ...
      // Gunakan gambar dari server jika ada, jika tidak, gunakan placeholder
      image:
              ...
              {latestPrograms.map((item, index) => (
                ...
                >
                  <div className="relative w-full h-32 md:h-40">
                    <Image
                      src={item.image} // Bisa berupa string URL atau objek gambar lokal
                      alt={item.title}
                      layout="fill"
                      objectFit="cover"
